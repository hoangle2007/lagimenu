-- Fix: error: role "kivomenu" does not exist (drizzle-kit / backend cần user này)
--
...
--   psql -h 127.0.0.1 -p 5432 -U postgres -f src/db/scripts/create-kivomenu-role.sql
--
...
DO $body$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'kivomenu') THEN
    CREATE ROLE kivomenu WITH LOGIN PASSWORD 'Hoang2007';
  END IF;
END
$body$;
...
CREATE DATABASE kivomenu OWNER kivomenu;
